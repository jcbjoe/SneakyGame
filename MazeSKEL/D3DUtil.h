...
void CreateVertexBuffer(UINT bufferSize, const void *pSourceData, ID3D11Buffer* &pVB, bool dyanmic = false);
...
void CreateIndexBuffer(UINT bufferSize, const void *pSourceData, ID3D11Buffer* &pIB, bool dynamic = false);

//update a dynamic buffer - e.g. a vertex or index buffer
//the cpy function should memcpy the data into the given buffer so be careful it's the correct size!
...
//given a 2D normal (a direction), convert it into degrees
float GetDegrees2D(const DirectX::SimpleMath::Vector2& normal);
//and back again
void SetDegrees2D(DirectX::SimpleMath::Vector2& normal, float deg);
//rotate the vector obviously
void RotateVector(DirectX::SimpleMath::Vector2& vec, float deg);
...
*/
float MinDist(const DirectX::SimpleMath::Vector2 &e1, const DirectX::SimpleMath::Vector2 &e2, const DirectX::SimpleMath::Vector2 &p, DirectX::SimpleMath::Vector2 &pointOnLine);
